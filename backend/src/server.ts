...
const getAdminDashboardSnapshot = (gameId?: string) => {
  ...
  const entriesWithHistory = rows.map((entry) => ({ ...entry, delayHistory: snapshotsByJourney.get(entry.trainId) ?? [] }));
