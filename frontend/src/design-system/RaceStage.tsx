  ...
  return <div className={`race-stage ${className}`.trim()}>
    ...
    <div className="admin-race-stage" style={{ "--race-row-count": sortedEntries.length } as CSSProperties}>
      {sortedEntries.map((entry, index) => {
        ...
        const status = cancelled ? "CANCELLED · WINNER" : entry.status === "arrived" ? "Arrived" : entry.status === "in_progress" ? remainingMinutes === null ? "🏁 unavailable" : `🏁 in ${remainingMinutes} min` : entry.stale ? "Live data stale" : departureMinutes === null ? "🚂 Waiting for departure" : `🚂 in ${departureMinutes} min`;
