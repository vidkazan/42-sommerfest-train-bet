...
export function ReplayBadge({ active = false, replayTimestamp, onSkip }: { active?: boolean; replayTimestamp?: number; onSkip: () => void }) {
  const replayTime = replayTimestamp === undefined ? null : new Date(replayTimestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  if (!active || !replayTime) return null;
  return <Badge variant="yellow" className="ds-replay-badge"><span className="ds-replay-badge__time" key={replayTimestamp} role="status">Replay {replayTime}</span><button type="button" className="ds-replay-badge__close" onClick={onSkip} aria-label="Skip replay">×</button></Badge>;
}
