...
import { Button, Notice, StatusBadge, TrainIcon } from "./components";
import { TimeLabelView } from "./TimeLabelView";
import { JourneyCard } from "./JourneyCard";
import { JourneyCell } from "./JourneyCell";

export type PublicView = "browse" | "progress" | "result";
...
export type TrainMapViewProps = { journeys: Journey[]; selectedTrainId: string | null; liveEntries: LiveLeaderboardEntry[]; onSelect: (trainId: string) => void };
...
export type LiveLeaderboardViewProps = { entries: LiveLeaderboardEntry[]; currentParticipantId: string | null; lastUpdatedAt: string | null; stale: boolean };
...
export function BetView({ journeys, selectedTrainId, username, betSubmitted, loading, error, onSelectTrain, onUsernameChange, onSubmit }: BetViewProps) {
  return <>
    ...
    {!betSubmitted && <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
      <label className="field-label" htmlFor="username">Username</label>
      <input id="username" value={username} onChange={(event) => onUsernameChange(event.target.value)} minLength={2} maxLength={24} placeholder="Your name" required />
      ...
      <Button type="submit" disabled={loading || !selectedTrainId}>{loading ? "Submitting…" : "Confirm my bet"}</Button>
    </form>}
    <div className="journey-list">
      {journeys.map((journey) => <JourneyCell key={journey.id} journey={journey} selected={selectedTrainId === journey.id} disabled={betSubmitted} onSelect={(selectedJourney) => onSelectTrain(selectedJourney.id)} />)}
    </div>
  </>;
}

export function LiveLeaderboardView({ entries, currentParticipantId, lastUpdatedAt, stale }: LiveLeaderboardViewProps) {
  return <section className="progress-view" aria-label="Live progress">
    <h2>Live progress</h2>
    <Notice>{stale ? "Live data is temporarily stale." : "Updates every minute."}</Notice>
    {lastUpdatedAt && <p>Last update: {new Date(lastUpdatedAt).toLocaleTimeString()}</p>}
    {!entries.length ? <p>No bets yet.</p> : <div className="journey-list" aria-label="Live leaderboard">{entries.map((entry) => {
      ...
      return <JourneyCard key={entry.trainId} journey={journey} mode="leaderboard" position={entry.position} bettors={entry.bettors} currentParticipantId={currentParticipantId} />;
    })}</div>}
  </section>;
}
...
export function AdminReviewView({ game, journeys, minimumDuration, selectedJourneyIds, loading, whitelistSaved, error, onFetch, onMinimumDurationChange, onToggleJourney, onSave, onActivate }: AdminReviewViewProps) {
  return <>
    ...
    {journeys.length > 0 && <>
      ...
      <div className="journey-list" aria-label="Candidate journeys">{journeys.filter((journey) => journey.durationSeconds >= Number(minimumDuration) * 3600).map((journey) => <JourneyCell key={journey.externalTripId} journey={journey} mode="admin" selected={selectedJourneyIds.includes(journey.externalTripId)} onToggle={(selectedJourney) => onToggleJourney(selectedJourney.externalTripId)} />)}</div>
...
export function TrainMapView({ journeys, selectedTrainId, liveEntries, onSelect }: TrainMapViewProps) {
  ...
  return <MapContainer className="train-map" center={center} zoom={8} scrollWheelZoom={false}>
    ...
    {[...routes].sort((a, b) => Number(a.journey.id === selectedTrainId) - Number(b.journey.id === selectedTrainId)).map(({ journey, points, endpoints }) => {
      ...
      return <Fragment key={journey.id}>
        <Polyline positions={positions} pathOptions={{ color: selected ? colors.map.routeSelected : colors.map.route, weight: selected ? 6 : 3, opacity: selected ? 1 : 0.85 }} eventHandlers={{ click: () => onSelect(journey.id) }}>
          <Popup>{journey.displayName}: {journey.origin} → {journey.destination}</Popup>
        </Polyline>
        {endpoints.slice(-1).map((point) => <CircleMarker key={`${journey.id}-arrival`} center={[point.lat, point.lon]} radius={4} pathOptions={{ color: colors.map.station, fillColor: colors.map.station, fillOpacity: 1 }}><Popup>Arrival: {journey.destination}</Popup></CircleMarker>)}
        {markerPoint && !cancelled && <CircleMarker center={[markerPoint.lat, markerPoint.lon]} radius={8} pathOptions={{ color: selected ? colors.map.trainSelectedBorder : colors.map.trainBorder, fillColor: selected ? colors.map.trainSelected : colors.map.train, fillOpacity: 1 }}><Popup><strong>{journey.displayName}</strong><br />{delaySeconds === null || delaySeconds === undefined ? "Delay unavailable" : `${Math.round(delaySeconds / 60)} min delay`}</Popup></CircleMarker>}
      </Fragment>;
    })}
  </MapContainer>;
}
